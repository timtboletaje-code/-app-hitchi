...
async function showDashboard() {
  ...
  render(`
    ...
    <div class="filtros" style="padding:8px 12px;background:#fff;border-bottom:1px solid #ddd">
      ...
      <div style="display:flex;gap:6px;margin-top:6px">
        <button onclick="descargarExcelCompleto()" class="btn-secondary" style="flex:1;text-align:center;padding:7px;border-radius:6px;font-size:11px;background:#555;color:#fff;border:none;cursor:pointer">📊 Excel completo</button>
      </div>
    </div>
...
async function saveForm(isEdit) {
  ...
  try {
    if (isEdit) {
      ...
      if (envData.success) alert('✅ Reporte guardado. PDF generado para envío manual.');
...
async function showDetail(folio) {
  ...
  try {
    ...
    html += `<div class="detail-actions">
        ...
        <button class="btn-success" onclick="descargarPDF('${folio}')" style="display:block;width:100%;text-align:center;padding:11px;border-radius:8px;margin-bottom:4px;border:none;cursor:pointer">📄 DESCARGAR PDF</button>
        <button class="btn-secondary" onclick="descargarExcel('${folio}')" style="display:block;width:100%;text-align:center;padding:11px;border-radius:8px;margin-bottom:4px;background:#555;color:#fff;border:none;cursor:pointer">📊 DESCARGAR EXCEL</button>
        <button class="btn-secondary" style="background:#333" onclick="descargarPDF('${folio}')">📄 Generar PDF para enviar</button>
      </div>`;
